...
JS는 싱글스레드 이기때문에 무거운 작업 하나를 구동하는동안, 리액트와 관련된 모든 상호작용이 정지합니다. 그 동안 리액트는 사용자의 입력에 응답하지 않으며 사용자는 불쾌한 경험을 하게 됩니다. 이 현상을 **반응성 중단(responsive block)**이라고 부르겠습니다.

### 예시로 필요성 이해하기

App.js에서 `<TabButton>` 중 클릭된 버튼에 해당하는 내용을 화면에 보여주는 리액트 코드입니다.
`About`, `Posts(slow)`, `Contact` 버튼 클릭시 각 버튼에 할당된 내용이 하단에 렌더링됩니다.
...
https://github.com/user-attachments/assets/fa3a583f-95dd-4874-beea-feba00de54f2

**반응성 중단**이 일어난 동안, 탭 버튼 컴포넌트는 About에 마우스를 올려도 hover UI를 보여주지 않는 문제가 있습니다.
또, 버튼을 연타할 동안 아무 반응이 없다가 Posts렌더링이 끝나야 About이 선택된다는 문제를 가지고 있습니다.
...
단순히 SlowList와 같이 컴포넌트의 양(count)이 많은 경우 뿐 아니라,
서버의 응답을 받아오는 네트워크요청이 발생하는 경우에도 서버의 응답을 받아오는 동안 React가 기다려야합니다.

## React의 동시성렌더링 사용하기

우선, 동시성렌더링의 핵심은 컴포넌트를 비동기적으로 렌더링 시킬 수 있다는 것입니다.
컴포넌트를 비동기적으로 렌더링하기 위해 `startTransition`과 `useDeferredValue` 두 가지 방법이 존재합니다.
Transitions
...
https://github.com/user-attachments/assets/1cda98fe-6a09-40dd-aece-55c3af7ad51f
